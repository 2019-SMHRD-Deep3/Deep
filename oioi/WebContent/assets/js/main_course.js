...
(function($) {
	...
	if (!browser.canUse('object-fit'))
		(function() {
			...
			$('.banner .image, .spotlight .image')
					.each(
							function() {

								var $this = $(this), $img = $this
										.children('img'), positionClass = $this
										.parent().attr('class').match(
												/image-position-([a-z]+)/);
	...
	$('.gallery')
			...
			//.css('overflow', 'hidden')
			.css('overflow-y', browser.mobile ? 'visible' : 'hidden').css(
					'overflow-x', browser.mobile ? 'scroll' : 'hidden')
			.scrollLeft(0);
	...
	$('.gallery')
			.on('wheel', '.inner', function(event) {
				...
				var $this = $(this), delta = (event.originalEvent.deltaX * 10);
			...
			.on(
					...
					function(event) {
						...
						var $this = $(this), $inner = $this.siblings('.inner'), direction = ($this
								.hasClass('forward') ? 1 : -1);
	...
	$('.gallery.lightbox')
			.on(
					...
					function(event) {
						...
						var $a = $(this), $gallery = $a.parents('.gallery'), $modal = $gallery
								.children('.modal'), $modalImg = $modal
								.find('img'), href = $a.attr('href');

						// Not an image? Bail.
						if (!href.match(/\.(jpg|gif|png|mp4|JPG)$/))
							return;
